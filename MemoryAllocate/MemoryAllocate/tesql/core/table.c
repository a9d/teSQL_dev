...
//addrLen - длина пол€ адресс в байтах
//sizeLen - длина пол€ размер в байтах
//aligment - выравнивание пам€ти в байтах
//startAddr - адрес начала таблицы
//endAddr - адрес конца таблицы
//Read - указатель на функцию чтени€
//Write - указатель на функцию записи
//Crc - указатель на функцию CRC, если используетс€
...
UINT8_T	memory_init(TableInfo* tab_ptr)
{
	...
	if(err == ERR_OK)
	{
		...
		if(err == ERR_OK)
		{
			...
			//Ѕлок старт +  онечный блок. “екущий не учитываетс€, т.к. он перезаписываетс€
...
UINT8_T WriteBlockLink(TableInfo* tab_ptr, BlockLink *bl)
{
	...
	//сформировать пакет дл€ записи и сгенерить crc16 если требуетс€
